...
# Physical label stock — die-cut, 29mm x 90mm (both dimensions fixed).
# Explicitly forced via DEVMODE at print time rather than trusting the
# driver's ambient/last-used settings.
LABEL_WIDTH_MM = 29
LABEL_HEIGHT_MM = 90
...
def compute_barcode_pieces(part_number: str, serial_number: str) -> list[str]:
    ...
    (pipe-separated) when both are present. Returns a 1-item list."""
...
def build_label_image(pieces: list[str], width_px: int, height_px: int) -> Image.Image:
    """
    Render one or more barcodes (Code128), with module width SOLVED to
    fill the label's fixed long dimension (die-cut stock — both width and
    length are fixed physical constants, LABEL_WIDTH_MM x LABEL_HEIGHT_MM
    — unlike continuous roll where only width is fixed).
    """
